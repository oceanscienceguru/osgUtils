...
gotoLoad <- function(inFile){
  with(inFile, {
    ...
    wptsRaw <- rawMa %>%
      ...
      dplyr::mutate(cleanNums = stringr::str_trim(nums)) %>% #clean up both sides
      ...
      dplyr::mutate(lat = gliderGPS_to_dd(rawlat),
                    long = gliderGPS_to_dd(rawlong)) %>%
      dplyr::mutate(rad = ifelse(!is_empty(rads), rads, 10)) %>% # pull in parsed radius if present. otherwise masterdata default
      dplyr::mutate(order = rownames(.)) #get order of wpts as listed
